...
public class GUIBlast extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(final float opacity, final int x, final int y) {
        ...
        int scale;
        if (blast.canSmelt()) {
            scale = blast.getSmeltProgressScaled(35);
            drawTexturedModalRect(xStart + 79, yStart + 35, 176, 0, scale, 15);
        }
    }
    ...
    protected void drawGuiContainerForegroundLayer(final int x, final int y) {
        final String containerName = blast.isInvNameLocalized() ? blast.getInvName() : StatCollector.translateToLocal(blast.getInvName());
